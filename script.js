...
function editar(numero) {
  ...
  if (numfila <= 1) {
    ...
    editar_gusto.innerHTML =
      "<input class='form - control' type='text' aria-label='default input example' size='15' name='gusto_data' id='gusto" +
      numero +
      "value='" +
      gusto_data +
      "'>";
    editar_porcentaje.innerHTML =
      "<input class='form - control' type='text' aria-label='default input example' size='3' name='porcentaje_data' id='porcentaje" +
      numero +
      "value='" +
      porcentaje_data +
      "'>";

    editar_fila.innerHTML =
      "<span style='color:#a29ea0; font-weight: 400;'>En edicion</span>";
    document.body.getElementsByClassName("div1")[0].textContent =
      "Pulse Aceptar para guardar los cambios o cancelar para anularlos";
    document.body.getElementsByClassName("div2")[0].innerHTML =
      '<button type="submit" class="btn btn-outline-secondary" id="aceptar" >Aceptar</button>';
    document.body.getElementsByClassName("div3")[0].innerHTML =
      '<button type="reset" class="btn btn-outline-danger" id="cancelar" onclick="location.reload()">Cancelar</button>';
  } else {
    alert(
      "Solo se puede editar una línea. Recargue la página para poder editar otra. SALUDOS PROFE"
    );
  }
}
